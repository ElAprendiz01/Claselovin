...
CREATE OR ALTER PROCEDURE sp_Tbl_Permisos_Opciones_Crear
(
    @Id_Rol INT,
    @Modulo NVARCHAR(50),
    @Puede_Crear BIT,
...
BEGIN
    ...
    BEGIN
        SET @o_code = -1;
        SET @o_message = 'Los valores de permisos (Crear, Leer, Actualizar, Eliminar) son obligatorios';
        RETURN;
    END;
    ...
    -- Validar duplicidad de rol y modulo
    IF EXISTS (SELECT 1 FROM Tbl_Permisos_Opciones WHERE Id_Rol = @Id_Rol AND Modulo = TRIM(@Modulo))
    BEGIN
        SET @o_code = -1;
    ...
    BEGIN TRY
        ...
        INSERT INTO Tbl_Permisos_Opciones
        (
            Id_Rol,
            Modulo,
            Puede_Crear,
        ...
        SET @o_code = 200;
        SET @o_message = 'Permiso creado correctamente';
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0
...
EXEC sp_Tbl_Permisos_Opciones_Crear
    @Id_Rol = 2,
    @Modulo = 'Reportes',
    @Puede_Crear = 0,
    @Puede_Leer = 1,
...
SELECT 
    ...
    @v_templateId AS PermisoIdGenerado;
GO
